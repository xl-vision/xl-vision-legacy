  ...
  return function () {
    ...
    return function (node, file) {
      ...
      for (let i = 0; i < demoboxs.length; i++) {
        ...
        const fileAbsPath = path.resolve(file.dirname, codePath)

        ctx.addDependency(fileAbsPath)
        ...
        demo.type = 'jsx'
        demo.value = `<DemoBox title={${title}} desc={<>${desc}</>} code={${code}} content={${JSON.stringify(
          content
        )}}><Demo${i} /></DemoBox>`
      }
    }
...
const highlightCode = (language, content) => {
  ...
  return converter.convert(
    `<pre class="language-${language}"><code class="language-${language}">${highlight(
      content,
      grammar,
      language
    )}</code></pre>`
