...
export default () => {
  ...
  const createElement = React.useCallback(() => {
    ...
    return (
      <div key={key} className='tg_item' onClick={() => removeElement(key)}>
  ...
  const addElement = React.useCallback(() => {
    setElements(prev => {
      ...
      return [...prev.slice(0, index), ele, ...prev.slice(index)]
    })
  }, [createElement])
  ...
  return (
    <div>
      ...
      <div className='tg_list'>
        <TransitionGroup classNames='tg_item'>{elements}</TransitionGroup>
      </div>
      <style jsx global>{`
        .tg_list {
          position: relative;
