# 加载中

用于页面与区块的加载中状态

:::demo 基本用法

一个简单的 loading 状态

```jsx
import { Spin } from 'xl-vision'
export default function() {
  return <Spin />
}
```

:::

:::demo 包含内容

将内容内嵌在`Spin`中。

```jsx
import { Spin } from 'xl-vision'
export default function() {
  return (
    <Spin>
      <div className='spin-inner'>Further details about the context of this alert.</div>
      <style jsx>{`
        .spin-inner {
          position: relative;
          width: 25rem;
          height: 10rem;
          background-color: #ccc;
          text-align: center;
          line-height: 10rem;
        }
      `}</style>
    </Spin>
  )
}
```

:::

:::demo 覆盖父元素

使用`cover`属性，可以使`Spin`覆盖父组件。需要注意的是父组件需要使用`position:relative`，否则无法生效。

```jsx
import { Spin } from 'xl-vision'
export default function() {
  return (
    <div className='spin-inner'>
      Further details about the context of this alert.
      <Spin cover />
      <style jsx>{`
        .spin-inner {
          position: relative;
          width: 25rem;
          height: 10rem;
          background-color: #ccc;
          text-align: center;
          line-height: 10rem;
        }
      `}</style>
    </div>
  )
}
```

:::

:::demo 不同大小

小的用于文本加载，默认用于卡片容器级加载，大的用于页面级加载。

```jsx
import { Spin } from 'xl-vision'
export default function() {
  return (
    <div className='spin-wrapper'>
      <Spin size='small' />
      <Spin size='default' />
      <Spin size='large' />
      <style jsx>{`
        .spin-wrapper :gloabl(.xl-spin) {
          margin-right: 1rem;
        }
      `}</style>
    </div>
  )
}
```

:::

:::demo 自定义描述

使用`tip`属性自定义描述文字

```jsx
import { Spin } from 'xl-vision'
export default function() {
  return <Spin tip='自定义加载文字' />
}
```

:::

:::demo 自定义指示器

通过`indicator`自定义加载指示器。

```jsx
import { Spin, Icon } from 'xl-vision'
export default function() {
  return <Spin indicator={<Icon.FasCircleNotch spin />} />
}
```

:::

:::demo 延时加载

通过`delay`延时显示加载器，防止闪烁。

```jsx
import { Spin, Button } from 'xl-vision'
export default function() {
  const [display, setDisplay] = React.useState(false)
  return (
    <div className='spin-wrapper'>
      <Spin delay={1000} spinning={display}>
        <div className='spin-inner'>Further details about the context of this alert.</div>
      </Spin>
      <Button onClick={() => setDisplay(!display)}>{display ? '关闭' : '打开'}</Button>
      <style jsx>{`
        .spin-inner {
          position: relative;
          width: 25rem;
          height: 10rem;
          background-color: #ccc;
          text-align: center;
          line-height: 10rem;
        }
        .spin-wrapper :gloabl(.xl-button) {
          display: block;
          margin-top: 1rem;
        }
      `}</style>
    </div>
  )
}
```

:::

## Spin API

| 参数 | 说明 | 类型 | 可选值 | 默认值 | 是否必填 |
| --- | --- | --- | --- | --- | --- |
| prefixCls | 自定义类名前缀 | string | - | - | false |
| cover | 覆盖父元素，此时父元素的 position 需要为`absolute`或者`relative` | boolean | - | false | false |
| delay | 延迟显示加载效果的时间（防止闪烁） | number(毫秒) | - | - | false |
| indicator | 自定义加载指示器 | ReactElement | - | - | false |
| size | 组件大小 | string | small &#x007c; default &#x007c; large | default | false |
| spinning | 是否正在加载中 | boolean | - | true | false |
| tip | 加载描述 | string | - | - | false |
| wrapperClassName | `indicator`和`tip`的包裹元素的自定义 class | string | - | - | false |
