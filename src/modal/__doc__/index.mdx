...
export default () => {
  ...
  return (
    <div>
      <Button onClick={click}>Open</Button>
      <Modal visible={visible} onVisibleChange={onVisibleChange} title='基础用法' getContainer={null}>
        <p>Some contents...</p>
        <p>Some contents...</p>
        <p>Some contents...</p>
      </Modal>
...
使用`create`直接创建，需要注意的是`create`创建的 modal 需要自己进行销毁，一般情况下建议**不要**使用此方式
...
| getContainer | 指定父容器,`null`表示挂载在当前dom | HTMLElement &#x007c; Selectors &#x007c; null &#x007c; (() => HTMLElement &#x007c; Selectors &#x007c; null) | - | () => document.body | false |
