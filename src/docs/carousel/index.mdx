...
```jsx
import { Carousel } from 'xl-vision'
export default () => {
  return (
...
使用`autoPlay`和`autoPlayDuration`设置自动播放与自动播放延时
...
设置`direction`为`vertical`可以竖直方向展示
