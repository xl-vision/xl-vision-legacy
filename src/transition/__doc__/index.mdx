...
通过传入`beforeEnter`、`enter`、`afterEnter`、`enterCancelled`、`beforeLeave`、`leave`、`afterLeave`、`leaveCancelled`等钩子函数，触发相应的动画。
通过改变属性`show`触发进出场钩子
...
设置`isAppear`，并默认开启动画时，会自动触发入场动画。
默认使用`beforeEnter`、`enter`、`afterEnter`、`enterCancelled`钩子。
如果需要特殊处理，可使用`beforeAppear`、`apper`、`afterAppear`、`appearCancelled`钩子。
...
默认隐藏时，会销毁`Transition`的子节点，通过`forceRender`强制隐藏状态下仍然保留子节点。
**此属性在需要保留子节点状态时非常有用。**
